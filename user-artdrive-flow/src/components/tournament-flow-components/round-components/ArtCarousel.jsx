...
const ArtCarousel = ({
    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center max-w-xl mx-auto">
            <div className="relative w-full overflow-hidden h-[500px] flex items-center justify-center z-30">
                <AnimatePresence initial={false}>
                    <motion.div
                        key={images[currentIndex].id}
                        className="absolute w-[93%] h-full z-50 overflow-hidden"
                        initial={{ x: 0, opacity: 1 }}
                        animate={{
                            x: swipeDirection === 1 ? 300 : swipeDirection === -1 ? -300 : 0,
                            opacity: swipeDirection ? 0 : 1,
                        }}
                        exit={{ x: swipeDirection === 1 ? 300 : swipeDirection === -1 ? -300 : 0, opacity: 0 }}
                        transition={{ duration: 0.3 }}
                        {...swipeHandlers}
                    >
                        <Image
                            ...
                            width={600}
                            height={800}
                            className="rounded-xl absolute object-cover bg-purple-950 z-50 animate-bg-pulse h-[500px] w-full"
                        />
                    </motion.div>
                </AnimatePresence>
                {loading && (
                ...
                <div className="absolute top-3 right-3 z-50">
                    <button
                        className="p-2 mr-3 bg-gray-800 border-[1px] h-[35px] w-[35px] flex items-center justify-center border-gray-400 bg-opacity-50 rounded-full"
                        onClick={handleExpand}
                    >
                        <Image src={expand} alt="expand" />
                    </button>
            ...
            <div className="flex relative justify-between space-x-16 mt-10 mb-[250px] z-50">
                <button
                    onClick={handleDislike}
                    className="w-[65px] h-[60px] flex items-center justify-center bg-white bg-opacity-10 rounded-3xl border-[1px] border-gray-500"
                >
